...
class InvitationsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val invitationsList = roomDatabase.getInvitacionDao().getAllInvitationsByUserID("raulhotmailcom")
        val userNameList = mutableListOf<String>()

        for (invitation in invitationsList) {
            val userName = roomDatabase.getUsuarioDao().getUsuarioByID(invitation.idUsuario).username
            val listName = roomDatabase.getListaDao().getListByID(invitation.idLista).title
            userNameList.add("$listName by $userName")
        }
        ...
        if (view is RecyclerView) {
            with(view) {
                ...
                adapter = MyInvitationsRecyclerViewAdapter(userNameList, listener)
            }
        }
        return view
    }
